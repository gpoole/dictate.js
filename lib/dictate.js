(function(window){
		...
		this.init = function(cb, err) {
			...
			try {
				...
				if(typeof err == 'function') {
					err(ERR_CLIENT, "Error initializing Web Audio browser: " + e);
				}
			}

			if (navigator.getUserMedia) {
				navigator.getUserMedia({audio: true}, startUserMedia.bind(window, cb), function(e) {
					config.onError(ERR_CLIENT, "No live audio input in this browser: " + e);
					if(typeof err == 'function') {
						err(ERR_CLIENT, "No live audio input in this browser: " + e);
					}
				});
			} else {
				config.onError(ERR_CLIENT, "No user media support");
				if(typeof err == 'function') {
					err(ERR_CLIENT, "No user media support");
				}
			}
		}
    ...
    this.submitReference = function submitReference(text, successCallback, errorCallback) {
      ...
      $.ajax({
		...
		// Private methods
		function startUserMedia(cb, stream) {
			stream = stream || cb;
