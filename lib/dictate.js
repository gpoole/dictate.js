(function(window){
	...
	// Grab prefixed WebRTC features
	var AudioContext = window.AudioContext || window.webkitAudioContext;
	var getUserMedia = navigator.getUserMedia || navigator.webkitGetUserMedia || navigator.mozGetUserMedia;
	if(getUserMedia) {
		// GUM must be called with navigator as the context to avoid Illegal invocation error
	...
	var Dictate = function(cfg) {
		...
		var wsServerStatus;
		// Initialized by startUserMedia
		var userSpeechAnalyser;
		...
		this.init = function() {
			...
			if (getUserMedia) {
				...
				getUserMedia(audioSourceConstraints, startUserMedia, function(e) {
    ...
    this.submitReference = function submitReference(text, successCallback, errorCallback) {
      ...
      $.ajax({
		...
		function startUserMedia(stream) {
			...
			userSpeechAnalyser = audioContext.createAnalyser();
